...
export const useSkewSlider = () => {
  ...
  const navigate = (direction: number) => {
    ...
    if (direction === 1) {
      nextSlide = currentSlideRef.current < slidesTotalRef.current - 1 ? currentSlideRef.current + 1 : 0;
    } else {
      nextSlide = currentSlideRef.current > 0 ? currentSlideRef.current - 1 : slidesTotalRef.current - 1;
    }
