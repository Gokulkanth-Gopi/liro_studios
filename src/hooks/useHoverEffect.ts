...
// Type for HoverEffect with optional next/previous methods
type ExtendedHoverEffect = HoverEffect & {
  next?: () => void;
  previous?: () => void;
  destroy?: () => void;
}
...
export const useHoverEffect = () => {
  ...
  const animationsRef = useRef<ExtendedHoverEffect[]>([]);
    ...
    try {
      ...
      for (const item of validItems) {
        ...
        const animation = new HoverEffect({
          parent: hoverImg,
          intensity: Number.parseFloat(hoverImg.dataset.intensity || '0.6'),
          speedIn: Number.parseFloat(hoverImg.dataset.speedin || '1'),
          speedOut: Number.parseFloat(hoverImg.dataset.speedout || '1'),
        ...
        const parentItem = item.closest('.tp--hover-item') as HTMLElement;
        if (parentItem) {
          const handleMouseEnter = () => {
            if (animation && typeof animation.next === 'function') {
              animation.next();
            }
          };
          const handleMouseLeave = () => {
            if (animation && typeof animation.previous === 'function') {
              animation.previous();
            }
          };
          ...
          // Store event listeners for cleanup in a Map instead of dataset
          handlersRef.current.set(parentItem, { enter: handleMouseEnter, leave: handleMouseLeave });
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Cleanup event listeners using the stored Map
      handlersRef.current.forEach((handlers, parentItem) => {
        if (parentItem) {
          parentItem.removeEventListener('mouseenter', handlers.enter);
          parentItem.removeEventListener('mouseleave', handlers.leave);
        }
      });
...
export const useHoverEffectTwo = (
  ...
  const hoverEffectRef = useRef<ExtendedHoverEffect | null>(null);
    ...
    const initHoverEffect = () => {
      ...
      try {
        ...
        eventListenersRef.current.mouseEnter = () => {
          if (hoverEffectRef.current && typeof hoverEffectRef.current.next === 'function') {
            hoverEffectRef.current.next();
          }
        };
        eventListenersRef.current.mouseLeave = () => {
          if (hoverEffectRef.current && typeof hoverEffectRef.current.previous === 'function') {
            hoverEffectRef.current.previous();
          }
        };
