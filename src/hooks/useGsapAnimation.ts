...
export const fadeAnimation = () => {
  gsap.utils.toArray<HTMLElement>(".tp_fade_anim").forEach((item) => {
    const tp_fade_offset = item.dataset.fadeOffset ? Number.parseInt(item.dataset.fadeOffset) : 40;
    const tp_duration_value = item.dataset.duration ? Number.parseFloat(item.dataset.duration) : 0.75;
    const tp_fade_direction = item.dataset.fadeFrom || "bottom";
    const tp_onscroll_value = item.dataset.onScroll !== "0";
    const tp_delay_value = item.dataset.delay ? Number.parseFloat(item.dataset.delay) : 0.15;
    const tp_ease_value = item.dataset.ease || "power2.out";

    let xValue = 0;
    let yValue = 0;

    if (tp_fade_direction === "left") {
      xValue = -tp_fade_offset;
    } else if (tp_fade_direction === "right") {
      xValue = tp_fade_offset;
    }

    if (tp_fade_direction === "top") {
      yValue = -tp_fade_offset;
    } else if (tp_fade_direction === "bottom") {
      yValue = tp_fade_offset;
    }
...
export const revalEffectAnimation = () => {
  document.querySelectorAll<HTMLElement>(".tp_reveal_anim").forEach(areveal => {
    const duration = areveal.dataset.duration || 1.5;
    const onScroll = areveal.dataset.onScroll || 1;
    const stagger = areveal.dataset.stagger ? Number.parseFloat(areveal.dataset.stagger) : 0.02;
    const delay = areveal.dataset.delay || 0.05;
...
export function animationParagraph() {
  const containers = gsap.utils.toArray<HTMLElement>('.tp_text_anim');
  containers.forEach((container) => {
    ...
    paragraphs.forEach((paragraph) => {
      ...
      // Remove prohibited aria-label from p tag (added by SplitText) to satisfy Lighthouse
...
export const textPerspectiveAnimation = () => {
  gsap.utils.toArray<HTMLElement>(".tp-text-perspective").forEach(splitTextLine => {
    const delayAttr = splitTextLine.dataset.delay;
    const delay_value = delayAttr ? Number.parseFloat(delayAttr) : 0.5;
    const tl = gsap.timeline({
      scrollTrigger: {
        trigger: splitTextLine,
        start: 'top 85%',
...
export const splitTextAnimation = () => {
  ...
  splitTextElements.forEach((el) => {
    ...
    // Create animation
    gsap.to(split.chars as gsap.TweenTarget, {
      scrollTrigger: {
        trigger: el,
        start: "top 90%",
        markers: false
...
const animateText = (t: HTMLElement, arr1: string[], arr2: string[]) => {
  const tl = gsap.timeline();
  let step: number = 0;
...
export const buttonBounceAnimation = () => {
  ...
  mybtn.forEach((btn) => {
    const tp_delay_value = btn.dataset.delay || 1;
...
export const contactBgAnimation = gsapBackgroundAnim;

//text moving animation
export const scrollMovingText = (scope?: Element | Document | null) => {
  const container = scope || document;
  gsap.utils.toArray<HTMLElement>('.moving-text', container).forEach((section, index) => {
