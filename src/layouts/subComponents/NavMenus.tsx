...
export default function NavMenus({ menuColor = "black" }: { readonly menuColor?: string }) {
  ...
  const renderSubmenu = (submenus: Submenu[] = []) => {
    return submenus.map((submenu, i) => {
      if (submenu.submenus) {
        return (
          <li key={submenu.title || i} className="menu-item-has-children">
            <Link href={submenu.link || "#"} aria-label={`Open ${submenu.title} submenu`}>{submenu.title}</Link>
      ...
      return (
        <li key={submenu.title || i}>
          <Link href={submenu.link || "#"} aria-label={`Go to ${submenu.title} page`}>
            {submenu.title}
          </Link>
        </li>
      );
              ...
              <div className="row gx-0">
                ...
                {menu.megaMenu &&
                  menu?.submenus?.map((submenu: Submenu, i: number) => (
                    <div key={submenu.title || i} className="col-xl-3">
                      <div className="tp-megamenu-list">
                        {submenu.title && (
                          <h2 className="tp-megamenu-title">{submenu.title}</h2>
                        )}
                        {submenu.megaMenu && (
                          <ul>
                ...
                {menu.smallMenu &&
                  menu?.submenus?.map((submenu: Submenu, i: number) => (
                    <div key={submenu.title || i} className="col-xl-4">
                      <div className="tp-megamenu-list">
                        {submenu.title && (
                          <h2 className="tp-megamenu-title">{submenu.title}</h2>
                        )}
