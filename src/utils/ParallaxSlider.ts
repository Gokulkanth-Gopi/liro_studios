...
export function parallaxSlider() {
  ...
  sliderWrapper.addEventListener('wheel', (e) => {
    ...
    if (sliderWidth) {
      ...
      target = Math.min(sliderWidth - globalThis.innerWidth, target);
    }
  });
  ...
  function init() {
    ...
    document.body.style.height = `${sliderWidth - (globalThis.innerWidth - globalThis.innerHeight)}px`;
  }

  function animate() {
    current = Number.parseFloat(lerp(current, target, ease).toFixed(2));
