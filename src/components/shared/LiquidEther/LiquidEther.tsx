...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    // IntersectionObserver and ResizeObserver setup
    ...
    try {
      ...
      console.warn('IntersectionObserver disconnect error:', e);
    }
    }
    if (webglRef.current) {
...
function initEngine(
  ...
  refs: {
    webglRef: React.MutableRefObject<any>;
  ...
  const { webglRef, isVisibleRef, rafRef } = refs;

function makePaletteTexture(stops: string[]) {
  let arr: string[];
  if (Array.isArray(stops) && stops.length > 0) {
    if (stops.length === 1) {
      arr = [stops[0] as string, stops[0] as string];
    } else {
      arr = stops;
    }
  } else {
...
class MouseClass {
  ...
  init(container: HTMLElement) {
    ...
    const defaultView = this.docTarget?.defaultView ?? globalThis.window ?? null;
...
class ExternalForce extends ShaderPass {
  ...
  override update(props: any) {
    ...
    const uniforms = (this.mouse?.material as THREE.RawShaderMaterial)?.uniforms as any;
    if (uniforms) {
...
class WebGLManager {
  ...
  dispose() {
    try {
      ...
      console.warn('LiquidEther dispose warning:', e);
    }
  }
}
